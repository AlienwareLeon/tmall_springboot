...
原型分为了前台原型和后台原型，原型图片已经放到项目文件夹(tmall_image)下面。
...
>分布式锁：
>数据库锁：
>前端防抖动（人话，防止重复调用保持数据一致性呵呵）：
>存储过程/存储过程的调用：
> 数据库回闪（恢复时间节点数据库数据呵呵，竟然遇到面试官拿这个做鄙视初级选手的点，吐了）
>Java内存模型：
>spring元注解/注解的原理/实现：
>redis集群：
>spring的生命周期：
>spring的核心组件和工作原理：
>mysql的锁机制：
>设计存储日志：ElasticSearch + Logstash + Kibana
...
按照模块之间的依赖关系，首先进行后台-分类管理功能开发。 查询，分页，增加，删除，编辑，修改。
